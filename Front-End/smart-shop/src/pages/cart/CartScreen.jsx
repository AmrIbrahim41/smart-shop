          ...
          <div className="flex-1 space-y-4">
            {cartItems.map((item) => (
              <div key={item.id} className="bg-white dark:bg-dark-accent p-4 rounded-2xl border border-gray-200 dark:border-white/5 flex items-center gap-4 shadow-sm dark:shadow-none transition-colors duration-300">
                <div className="w-24 h-24 bg-gray-100 dark:bg-white rounded-xl overflow-hidden flex-shrink-0 border border-gray-200 dark:border-none">
                    <img src={`http://127.0.0.1:8000${item.image}`} alt={item.name} className="w-full h-full object-contain mix-blend-multiply dark:mix-blend-normal" />
                </div>
                <div className="flex-1">
                    <Link to={`/product/${item.id}`} className="text-gray-900 dark:text-white font-bold text-lg hover:text-primary transition line-clamp-2">{item.name}</Link>
                ...
                <div className="flex items-center bg-gray-100 dark:bg-dark rounded-lg border border-gray-200 dark:border-white/10 transition-colors">
                    <button onClick={() => addToCart(item, Math.max(1, item.qty - 1))} className="p-3 text-gray-500 dark:text-gray-400 hover:text-dark dark:hover:text-white transition"><FaMinus size={10} /></button>
                    <span className="w-8 text-center text-gray-900 dark:text-white font-bold text-sm">{item.qty}</span>
                    <button onClick={() => addToCart(item, item.qty + 1)} className="p-3 text-gray-500 dark:text-gray-400 hover:text-dark dark:hover:text-white transition"><FaPlus size={10} /></button>
                </div>
                <button onClick={() => removeFromCart(item.id)} className="p-3 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-500 transition"><FaTrash /></button>
              </div>
            ))}
          </div>
          ...
          <div className="lg:w-[350px]">
            <div className="bg-white dark:bg-dark-accent p-6 rounded-3xl border border-gray-200 dark:border-white/10 sticky top-28 shadow-xl dark:shadow-none transition-colors duration-300">
                ...
                <div className="space-y-3 text-sm mb-6 text-gray-600 dark:text-gray-400">
                    <div className="flex justify-between"><span>{t('subtotal') || "Subtotal"}</span><span className="text-gray-900 dark:text-white font-bold">${subtotal.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span>{t('tax') || "Tax"}</span><span className="text-gray-900 dark:text-white font-bold">${tax.toFixed(2)}</span></div>
                    <div className="flex justify-between"><span>{t('shippingFee') || "Shipping"}</span><span className="text-gray-900 dark:text-white font-bold">${shipping}</span></div>
                </div>
                ...
                <button onClick={checkoutHandler} className="w-full bg-primary hover:bg-orange-600 text-white font-bold py-4 rounded-xl flex justify-center items-center gap-2 transition shadow-lg uppercase">
